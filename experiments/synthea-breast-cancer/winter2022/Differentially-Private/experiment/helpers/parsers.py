...
import re
from experiment.helpers import defaults
...
class UniqueInfoParser:
    ...
    def get_uniq_procedures(self) -> List[str]:
        ...
        for patient in self.patient_info_json:
            procs = [procedure.get('procedureType') for procedure in patient.get(
                'cancerRelatedProcedures', defaults.DEFAULT_PROCEDURES)]
    ...
    def get_uniq_meds(self) -> List[str]:
        meds = []
        for patient in self.patient_info_json:
            patient_meds = [med.get('medicationCode').get('label') for med in patient.get(
                'medicationStatement', defaults.DEFAULT_MEDS)]
...
class PatientInfoParser:
    ...
    def __encode_one_hot_age(self) -> Optional[int]:
        age = self.patient.get(
            'subject', defaults.DEFAULT_SUBJECT).get('dateOfBirth')
        diag = self.patient.get('cancerCondition', defaults.DEFAULT_CANCER_CONDITION)[
            0].get('dateOfDiagnosis')

        if age is not None and diag is not None:
            return self.__get_age(age, diag)
        return None
    ...
    def __encode_one_hot_status(self) -> Optional[int]:
        cancer_status = self.patient.get(
            'cancerDiseaseStatus', defaults.DEFAULT_LABEL)

        if cancer_status is not None:
            return self.__get_status(cancer_status.get('label'))
        return None
    ...
    def __encode_one_hot_stage(self) -> Dict[str, int]:
        ...
        cancer_stagings = self.patient.get(
            'cancerCondition', defaults.DEFAULT_CANCER_CONDITION)[0].get('tnmStaging')

        if cancer_stagings is not None:
            stage_group = cancer_stagings[0].get(
                'stageGroup', defaults.DEFAULT_GROUP)
            stage = self.__get_stage(stage_group.get(
                'dataValue', defaults.DEFAULT_LABEL).get('label', None))

            primary_group = cancer_stagings[0].get(
                'primaryTumorCategory', defaults.DEFAULT_GROUP)
            primary = self.__get_primary(primary_group.get(
                'dataValue', defaults.DEFAULT_LABEL).get('label', None))

            nodes_group = cancer_stagings[0].get(
                'regionalNodesCategory', defaults.DEFAULT_GROUP)
            nodes = self.__get_nodes(nodes_group.get(
                'dataValue', defaults.DEFAULT_LABEL).get('label', None))
    ...
    def __get_status(self, cancer_status: str) -> int:
        if cancer_status == "Patient's condition improved":
            return 1
        return 0
    ...
    def __get_stage(self, given_stage: str) -> Optional[int]:
        if re.search(r'[sS]tage 1', given_stage):
            return 1
        if re.search(r'[sS]tage 2', given_stage):
            return 2
        if re.search(r'[sS]tage 3', given_stage):
            return 3
        if re.search(r'[sS]tage 4', given_stage):
            return 4
        return None
    ...
    def __get_primary(self, given_stage: str) -> Optional[int]:
        if 'T0' in given_stage:
            return 0
        if 'T1' in given_stage:
            return 1
        if 'T2' in given_stage:
            return 2
        if 'T3' in given_stage:
            return 3
        if 'T4' in given_stage:
            return 4
        return None
    ...
    def __get_nodes(self, given_stage: str) -> Optional[int]:
        if 'N0' in given_stage:
            return 0
        if 'N1' in given_stage:
            return 1
        if 'N2' in given_stage:
            return 2
        if 'N3' in given_stage:
            return 3
        return None
